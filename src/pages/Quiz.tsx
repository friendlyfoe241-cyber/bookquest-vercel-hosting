...
import { Check, X, Star, MessageSquare, Zap, Coins, BookOpen, Loader2 } from 'lucide-react';
...
const Quiz = () => {
  ...
  // Check if there's more content to load for this book
  const remainingKey = `bookquest-remaining-${bookId}`;
  const remainingRaw = localStorage.getItem(remainingKey);
  ...
  const handleAnswer = (idx: number) => {
    ...
    setTimeout(() => {
      ...
      if (currentQ < book.quiz.length - 1) {
        ...
        const isReread    = progress.booksRead.filter((id: string) => id === book.id).length > 1
        ...
        const breakdown   = calcTotalBookCoins(difficulty, finalScore, book.quiz.length, qteData.passed, qteData.total, isReread);
        setCoinsBreakdown(breakdown);

        (async () => {
          const { data: { user } } = await supabase.auth.getUser();
          if (!user) return;
          const { data: prof } = await supabase.from('profiles').select('coins').eq('user_id', user.id).single();
          if (prof) {
            await supabase.rpc('update_profile_economy', { p_user_id: user.id, p_coins: prof.coins + breakdown.total } as any);
          }
          const { data: ub } = await supabase.from('user_books').select('id, coins_earned').eq('user_id', user.id).eq('book_id', book.id).maybeSingle();
          if (ub) await supabase.from('user_books').update({ coins_earned: (ub as any).coins_earned + breakdown.total } as any).eq('id', ub.id);
        })();
  ...
  // Load the next 50k-char chunk of the book as a new "Part N" entry
  const handleLoadMore = async () => {
    ...
    try {
      ...
      toast.success(`${partTitle} is ready!`);
  ...
  const handleRate = () => { rateBook(book.id, rating); navigate('/foryou'); };
      ...
      <div className="min-h-screen flex flex-col items-center justify-center p-8 bg-background">
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center">
          ...
          <div className="flex flex-col gap-3 mt-6">
            <Button className="rounded-2xl px-8" onClick={handleRate}>Done! →</Button>
            <Button variant="outline" className="rounded-2xl px-8" onClick={() => { rateBook(book.id, rating); navigate(`/reviews/${book.id}`); }}>
        ...
        <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: 'spring', bounce: 0.5 }}
          ...
          {coinsBreakdown && coinsBreakdown.total > 0 && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
              ...
              <div className="flex items-center justify-center gap-1 text-lg font-bold text-foreground mb-1">
                <Coins className="w-5 h-5 text-yellow-500" />+{coinsBreakdown.total} coins
              </div>
          ...
          <div className="flex justify-center gap-1 mb-5">
            {Array.from({ length: book.quiz.length }).map((_, i) => (
              <motion.div key={i} initial={{ scale: 0, rotate: -180 }} animate={{ scale: 1, rotate: 0 }} transition={{ delay: i * 0.2, type: 'spring' }}>
                <Star className={`w-8 h-8 ${i < score ? 'text-amber-400 fill-amber-400' : 'text-muted'}`} />
              </motion.div>
            ))}
          </div>

          {/* Continue Reading button — shows only if more text is available */}
          {hasMore && (
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}>
              <Button
                onClick={handleLoadMore}
                disabled={loadingMore}
                className="w-full rounded-2xl mb-3 bg-primary/90 hover:bg-primary"
              >
                {loadingMore ? (
                  <><Loader2 className="w-4 h-4 animate-spin mr-2" />Loading Part {(remaining?.partNumber || 1) + 1}…</>
                ) : (
                  <><BookOpen className="w-4 h-4 mr-2" />Continue Reading — Part {(remaining?.partNumber || 1) + 1} →</>
                )}
              </Button>
            </motion.div>
          )}

          <Button className="w-full rounded-2xl px-8" onClick={() => setShowRating(true)}>
            Rate This Book →
          </Button>
        </motion.div>
      </div>
